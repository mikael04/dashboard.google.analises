...
#   df_date_count <- data.table::read(paste0(getwd(),"/data-raw/df_date_count.csv"))
df_soc_pand <- data.table::fread("data-raw/perguntas_full.csv")
p <- collapsibleTree::collapsibleTree(df_soc_pand, c("EIXO","TOPICS", "QUERIES", "QUESTIONS"),
                                             attribute = "EIXO",
                                             width = 2000,
                                      collapsed = T,
                                      zoomable = T,
...
p
df <- df_soc_pand %>%
    dplyr::filter(TOPICS == "Prevention and Control") %>%
    dplyr::select(-EIXO, -TOPICS)
collapsibleTree::collapsibleTree(df, c("QUERIES", "QUESTIONS"),
                                 root = "Prevention and Control",
                                 width = 1400,
                                 collapsed = T,
                                 zoomable = T,
                                 tooltipHtml = "tooltip")
